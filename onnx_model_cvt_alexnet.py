...
#the model is downloaded from https://s3.amazonaws.com/download.onnx/models/opset_8/bvlc_alexnet.tar.gz 

file_name = './bvlc_alexnet/model.onnx'
try:
...
for initializer in INTIALIZERS:
    ...
    print(W.shape)
    if W.ndim == 4:
        if initializer.name == 'conv1_w_0':
            W = np.swapaxes(W, 2, 3)  #swap w,h
            W = W[:, ::-1, :, :]     #revert RGB channel
            print('transform conv1_w_0')
        else:
            W = np.swapaxes(W, 2, 3)
        ...
        if initializer.name == 'fc6_w_0':
            W = W.reshape(( W.shape[0] * W.shape[1])//36,6,6)  #swap w,h
            W = np.swapaxes(W, 1, 2)
